...
public class MainVerticle extends AbstractVerticle {
    ...
    private Future<Void> startServer(JsonObject config) {
        ...
        if (config.getBoolean(Constants.ENV_PIVEAU_HUB_LOAD_VOCABULARIES_FETCH, true)) {
            ConceptSchemes.enableRemotes();
        }
        ...
        OpenAPI3RouterFactory.create(vertx, "webroot/openapi.yaml", handler -> {
            if (handler.succeeded()) {
                ...
                //routerFactory.addHandlerByOperationId("postMetric", metricHandler::handlePostMetric);
    ...
    private Future<JsonObject> loadConfig() {
        ConfigStoreOptions envStoreOptions = new ConfigStoreOptions()
                ...
                .setConfig(new JsonObject().put("keys", new JsonArray()
                        ...
                        .add(Constants.ENV_PIVEAU_HUB_TRIPLESTORE_CONFIG)
    ...
    private Future<JsonObject> bootstrapVerticles(JsonObject config) {
        ...
        Promise<String> validationSevicePromise = Promise.promise();
        vertx.deployVerticle(ValidationServiceVerticle.class.getName(), new DeploymentOptions().setConfig(config).setWorker(true), validationSevicePromise);
        ...
        CompositeFuture.all(Arrays.asList(
                ...
                translationSevicePromise.future(),
                validationSevicePromise.future())).setHandler(ar -> {
            if (ar.succeeded()) {
