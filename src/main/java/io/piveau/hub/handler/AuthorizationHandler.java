...
public class AuthorizationHandler {
    ...
    public void handleAuthorization(RoutingContext context) {
        ...
        if (context.data().get(AUTHENTICATION_TYPE).equals(JWT_AUTH)) {
            ...
            if (user == null) {
                ...
                switch (requestMethod) {
                    case PUT:
                        /**** if request is a put dataset (create dataset) check if user has permission to update the catalogue in which the dataset resides ****/
                        if (context.request().query() != null) { // context.request().query().contains("datasets").contains("catalogue") -> if other PUT requests will be used
                            ...
                            if (catalogueID != null) {
                                ...
                                for (String key : userResources) {
                                    if (key.contains(catalogueID) == true) {
                                        ...
                                        if (userPermissionsAndResources.get(key).contains(UPDATE)) {
                                            log.debug("PUT dataset with RTP went through");
                                            context.next();
                                        } else {
                                ...
                                if (resource == false) {
                                    ErrorCodeResponse.forbidden(context, "No permission for this resource (after for loop)");
                                }
                            } else {
                            ...
                            if (catalogueID != null) {
                                ...
                                for (String key : userResources) {
                                    if (key.contains(catalogueID) == true) {
                                        resource = true;
                                        if (userPermissionsAndResources.get(key).contains("update")) {
                                            log.debug("DELETE dataset with RTP went through");
                                            context.next();
                                        } else {
                                            ErrorCodeResponse.forbidden(context, "No permission for this resource");
                                        }
        ...
        /**** if authentication was done with an API key, no authorization process is necessary, so move along ****/
            ...
            if (context.request().method().toString().equals(PUT)) {
                /**** PUT catalogue ****/
